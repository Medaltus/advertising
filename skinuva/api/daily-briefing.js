...
// Runs daily at 9am ET. Fetches Skinuva sheet data, generates an executive
...
const SHEET_ID = process.env.SHEET_ID || '1CITWFOhGXFSyXFE0j-Rze-oNfmmDqHXFUUSHGDeA7UI';
...
You are reviewing Skinuva's advertising performance across channels (Amazon, Google, Meta, etc.). The data below is raw pipe-delimited table data from Google Sheets covering multiple months. Find ONLY the ${currentMonth} data and generate a quick morning briefing for our Slack channel.

Rules:
- Plain language, like a casual update to a manager
- Under 200 words
- Lead with the bottom line: is Skinuva on track or not vs the 30% ACOS goal?
- Key numbers: total ad spend, ad sales, ACOS, and which channels are working or not
...
${rawData.substring(0, 18000)}`;
    ...
    const slackMessage = `*📊 Skinuva — ${today}*\n\n${briefing}`;
    ...
    try {
      await postToSlack(slackUrl, `⚠️ Skinuva morning briefing failed: ${err.message}`);
    } catch (_) {}
